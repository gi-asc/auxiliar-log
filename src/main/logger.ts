...
export class Logger implements ILogger{
    ...
    private formateLogAxiosRequest (url: string,token = '', params?: any, headers?: any, response?: any, functionName?: string, data?: any): string {
        ...
        token ? log = log + (`\n${nameFunc} ${clc.cyan(`TOKEN:::`)} ${clc.white(token)}`): log = log + (`\n${nameFunc} ${clc.cyan(`TOKEN:::`)} token is not provided`)
        data ? Object.keys(data).forEach((key) => {
          if(typeof data[key] == 'object') {
            log = log + (`\n${nameFunc} ${clc.cyan(`DATA(${key}):::`)} ${clc.white(JSON.stringify(data[key]))}`)
            } else {
        ...
        headers ? Object.keys(headers).forEach((key) => {
          log = log + (`\n${nameFunc} ${clc.cyan(`HEADERS(${key}):::`)} ${clc.white(headers[key])}`)
        }) : log = log + (`\n${nameFunc} ${clc.cyan(`HEADERS:::`)} headers is not provided`)
      ...
      private formateAxiosRequestError(url: string,token = '', params?: any, headers?: any, error?: any, functionName?: string, data?: any): string {
        ...
        token ? log = log + (`\n${nameFunc} ${clc.magentaBright(`TOKEN:::`)} ${clc.white(token)}`): log = log +(`\n${nameFunc} ${clc.magenta(`TOKEN:::`)} token is not provided`)
        data ? Object.keys(data).forEach((key) => {
          if(typeof data[key] == 'object') {
            log = log + (`\n${nameFunc} ${clc.magentaBright(`DATA(${key}):::`)} ${clc.white(JSON.stringify(data[key]))}`)
            } else {
              log = log + (`\n${nameFunc} ${clc.magentaBright(`DATA(${key}):::`)} ${clc.white(data[key])}`)
            }
        }) : log = log + (`\n${nameFunc} ${clc.magentaBright(`DATA:::`)} data is not provided`)
        params ? Object.keys(params).forEach((key) => {
          log = log + (`\n${nameFunc} ${clc.magentaBright(`PARAMS(${key}):::`)} ${clc.white(params[key])}`)
        }) : log = log + (`\n${nameFunc} ${clc.magentaBright(`PARAMS:::`)} params is not provided`)
        headers ? Object.keys(headers).forEach((key) => {
